...
@Service
@Command(scope = "onos", name = "qos-del-all",
        description = "Qos Test.")
public class QoSdelAll extends AbstractShellCommand {
	...
	if(queueConfig.getQueues().stream().count()==0) print("No Queues (empty) ! ");
                		...
                		queueConfig.deleteQueue(queue.queueId());
            	});
        }
	...
	qosConfig.getQoses().stream().forEach(qos ->{
		...
		//if(String.valueOf(qos.qosId()).equals(id)) {
			...
			portConfig.removeQoS(PortNumber.portNumber(1));
                        portConfig.removeQoS(PortNumber.portNumber(2));
