...
@Service
@Command(scope = "onos", name = "qos-test", description = "Qos Test.")
public class QoSAddCommandv2 extends AbstractShellCommand {

        @Argument(index = 0, name = "ovsdbDeviceId", description = "the ovsdb TYPE id.", required = true, multiValued = false)
        String ovsdbDeviceId = null;

        @Argument(index = 1, name = "qos id", description = "Qos id.", required = true, multiValued = false)
        String id = null;

        /*
         * @Argument(index = 2, name = "port name", description = "the port name.",
         * required = true, multiValued = false) String portName = null;
         */

        @Argument(index = 2, name = "port number", description = "the port number.", required = true, multiValued = false)
        String portnumber = null;

        @Argument(index = 3, name = "rate", description = "the rate of interface.", required = true, multiValued = false)
        String rate = null;

        @Argument(index = 4, name = "burst", description = "the burst of interface.", required = true, multiValued = false)
        String burst = null;

        @Argument(index = 5, name = "dscp", description = "Description service Code Point", required = false, multiValued = false)
        int dscp = 0; // low priority by default
        /*
         * @Argument(index = 6, name = "priority", description = "Priority", required =
         * false, multiValued = false) long priority = 0; // lowest priority by default
         */
        ...
        protected long availableQueueId(QueueConfigBehaviour queueConfig) {
                ...
                if (queueConfig.getQueues().isEmpty())
                        return 0;
                else {
                        ...
                        for (Map.Entry<String, QueueDescription> entry : treeQueues.entrySet()) {
                                ...
                                //////////////////
                                print("Map of Queueus, currentID to Test = %s , index[%s] = %s", entry.getKey(),
                                                String.valueOf(queueCounter.get()), index[0]);
                                ///////////////////
                                if (index[1].equals(id)) {
                                        if (Long.valueOf(index[0]) <= queueCounter.longValue())
                                                queueCounter.getAndIncrement();
                                        else {
                                                print("Condition met Getting Out from the loop ! ");
                                                break;
                                        }
                                }
                        }
        ...
        protected QosDescription qosDescCreator(String name, Map<Long, QueueDescription> longerQueues) {
                QosDescription qosDescription = DefaultQosDescription.builder().qosId(QosId.qosId(name))
                                .type(QosDescription.Type.HTB).maxRate(Bandwidth.bps(Long.valueOf("10000")))
                                .queues(longerQueues).build();
        ...
        protected void doExecute() {
                ...
                QueueDescription queueDesc = DefaultQueueDescription.builder().queueId(QueueId.queueId(queueID))
                                .maxRate(Bandwidth.bps(Long.parseLong(rate))).minRate(Bandwidth.bps(Long.valueOf(rate)))
                                .burst(Long.valueOf(burst)).dscp(dscp)
                                // .priority(priority)
                                .build();
                ...
                if (qosDesc.queues().isPresent()) {
                        ...
                        print("qosDesc.queues() ==> qoses.queues() = No queues");

                print("Addqueue");
                queueConfig.addQueue(queueDesc);
                print("queue added");
                if (!qosNoTExist) {
                        print("QoS exists !\nQoS ID (port)=%s, maxRate=%s, cbs=%s, cir=%s, " + "queues=%s, type=%s",
                                        qosDesc.qosId(), qosDesc.maxRate(), qosDesc.cbs(), qosDesc.cir(),
                                        qosDesc.queues(), qosDesc.type());
                ...
                qosConfig.getQoses().stream().forEach(qoses -> {
                        if (qoses.queues().isPresent()) {
                                ...
                                for (Map.Entry<Long, QueueDescription> entry : (qoses.queues().get()).entrySet()) {
                                        print("\nPhase 3 Queues Map :");
                                        print("Key : %d - Inside QueueId : %s ", entry.getKey(),
                                                        entry.getValue().queueId());
                                }
                        } else
                                print("qosConfig ==> qoses.queues() = No queues");
                });
        }
}
